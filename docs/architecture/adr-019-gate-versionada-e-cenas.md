...
**Adendo (ADR-021):** quando o job fecha `SUCCEEDED_PARTIAL`, o Variety Gate é reexecutado sobre o subconjunto efetivamente entregue com teto recomputado `ceil(D/K)` (D = entregues). Se o subconjunto viola o teto, o drop do excedente da função concentrada é **determinístico e total-order**: entre os itens da função concentrada, remove-se primeiro o de maior contagem de cheques reprovados (assinatura residual, gate + judge); em empate, o de menor número de `evidenceRefs` ancorados; em empate, o de maior posição no plan (posicional decrescente — preserva os primeiros e mantém o resultado estável entre execuções). Cada drop decrementa D, recomputa o teto e revalida; ao final, se F = N−D exceder `PARTIAL_FAILURE_CAP`, o job reprova inteiro (`FAILED`) — o teto é reavaliado APÓS os drops de variedade. A variedade aprovada é sempre a do conjunto publicado, nunca a do planejado.

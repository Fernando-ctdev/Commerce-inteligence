...
O mapa segue o core loop:

```text
Workspace → Product → Strategy → Plan → Content → Production
```

O primeiro valor do MVP é o creator conseguir indicar um produto do TikTok Shop, confirmar os fatos descobertos e chegar ao primeiro plano de conteúdo sem preencher um formulário estratégico.

Este documento é um mapa de construção. Não é SPEC, PLAN ou decisão arquitetural.

## Princípios usados para definir os slices

- **Comportamento antes de camada:** nenhum slice existe apenas para criar Database, Frontend, API, Auth, Worker, Browser Service ou infraestrutura.
- **Verticalidade suficiente:** cada slice atravessa os domínios necessários para uma capacidade real do creator.
- **Infraestrutura just-in-time:** Browser Harness entra no slice que entrega a importação; fila durável e Entitlements de geração entram no slice que entrega o primeiro resultado estratégico.
- **Fonte factual separada de estratégia:** Product Import descobre fatos; Commerce Intelligence decide estratégia. O cadastro não pede informações que a plataforma consegue descobrir ou que pertencem à engine.
- **Human-in-the-loop explícito:** login, CAPTCHA, QR Code, 2FA e confirmações do TikTok pertencem ao creator; a automação pausa e retoma depois.
...
- **UX orientada à próxima ação:** mobile mantém a experiência completa; desktop aumenta planejamento e densidade sem criar capacidade exclusiva.
- **Escopo explícito:** APIs oficiais, OAuth do TikTok, scraping universal, publicação, analytics externo, mídia gerada, colaboração, embeddings e integrações adicionais permanecem fora deste MVP.

## Jornadas principais

- **Ativação:** criar conta → colar URL → abrir TikTok no profile do usuário → resolver autenticação manual se necessário → extrair fatos → confirmar Product → gerar primeira Strategy/Plan.
- **Reutilização da sessão:** colar URL → reutilizar profile autenticado → extrair fatos → confirmar Product.
- **Recuperação:** importação falha → revisar erro → adicionar Product manualmente com nome e descrição.
- **Planejamento:** abrir Product → informar quantidade/objetivo → acompanhar geração → receber Strategy, Plan e Contents graváveis.
- **Execução:** revisar Content → colocar na fila → gravar → marcar como Gravado.
- **Recorrência:** consultar memória → gerar novo lote → priorizar lacunas → continuar Production.

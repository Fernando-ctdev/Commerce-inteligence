...
11. **O ciclo de vida inicial de Entitlements foi fechado no ADR-006:** todo Tenant provisionado recebe um entitlement default server-side, com limite `active_products` configurado no servidor; ausência ou invalidade da configuração bloqueia ativação sem fallback controlado pelo cliente. Troca de plano, cobrança, upgrade e downgrade permanecem fora do Slice 002.

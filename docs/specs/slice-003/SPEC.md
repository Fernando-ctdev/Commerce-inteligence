...
- Validação estrutural, factual e de variedade; `BriefValidationReport` por briefing; repair limitado e sem completar quantidade com conteúdo inválido.
...
Antes da publicação, cada briefing passa pelos hard gates objetivos estrutural, factual, de relação, cardinalidade e plataforma, e o conjunto passa pelo Variety Gate determinístico. Falha objetiva bloqueia o item/job conforme ADR-021. Depois, `CONTENT_QUALITY_JUDGE` avalia uma vez `hook`, `development`, `script`, `cta` e cenas por parte, retornando somente `PASS|REVIEW`. `PASS` preserva a parte; `REVIEW` aciona no máximo um `CONTENT_PART_REPAIR` somente para a parte marcada. Não há Judge na segunda passada. Se o repair falhar, retornar schema inválido ou não puder ser aplicado, a parte original permanece. Quando os hard gates objetivos passam, o Content é persistido em `DRAFT`, mesmo com fallback semântico.

Devem ser detectáveis campos obrigatórios ausentes, relações/IDs inválidos, quantidade incorreta, claims sem suporte ou contraditos, quantidade de cenas válida para o formato, duplicatas exatas/normalizadas, duplicata por hash de estrutura e concentração objetiva. O Judge cobre somente coerência com Produto, estilo/configuração declarados do creator e plataforma; não reavalia factualidade e não transforma preferência subjetiva em bloqueio.

O `BriefValidationReport` registra os resultados objetivos (`factualStatus`, `structuralStatus`, `platformStatus`, `varietyStatus`, `issues`) separadamente da auditoria semântica. Sua chave canônica é `briefId`, derivada de `contentId + briefVersionId`. A auditoria semântica registra `contentId`, round inicial, cinco partes, criterion e reason allowlisted; seus status são apenas `PASS|REVIEW`. Não existem `REJECT`, `unsupported_persuasion` ou `QUALITY_PENDING`.

Repair não altera IDs, ownership, quota, Strategy ativa ou quantidade do plano. A aplicação valida cada retorno individualmente e recompõe somente a parte marcada; erro preserva o original. Faltantes e `SUCCEEDED_PARTIAL` decorrem apenas de falha objetiva ou composição objetivamente inválida, nunca de `REVIEW` semântico.

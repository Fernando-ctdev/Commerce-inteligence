...
| Conflito sobre gates | Hard gates e Variety Gate permanecem determinísticos e fora do LLM. Após eles, `CONTENT_QUALITY_JUDGE` avalia hook, development, script, CTA e cenas em batches homogêneos e retorna somente `PASS|REVIEW`; cada `REVIEW` pode receber um repair único. Repair inválido preserva a parte original; hard gates finais decidem entrega. Conteúdo hard-válido publica em `DRAFT`; falha objetiva segue ADR-021. | Separa segurança/factualidade de qualidade editorial sem transferir regra de sistema ao LLM e sem criar estado público semântico. | Sim, por ADR-025 e ADR-029 |
...
30. **WHEN** o conjunto contiver duplicata exata/normalizada ou repetição estrutural indevida, **o sistema SHALL** acionar repair com as causas, preservar Briefings `PASS` e não inventar oportunidade irrelevante; sem Variety Gate válido, não poderá publicar.

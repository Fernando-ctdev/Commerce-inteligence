...
- `docs/architecture/adr-027-publicacao-quality-pending.md`;
...
- Entregar a primeira `ProductStrategy` e `targetContentCount` Contents publicáveis após um job bem-sucedido: `DRAFT` quando o Judge semântico conclui `PASS`, ou `QUALITY_PENDING` somente na exceção fechada do ADR-027. Quando parte do lote for realmente não publicável dentro do teto de falhas, o job conclui `SUCCEEDED_PARTIAL`, com variedade revalidada e retry explícito dos faltantes (ADR-021).
- Permitir navegação contínua e reentrada sem perder o estado real do job.
- Impedir sucesso parcial silencioso, consumo duplicado, duplicação de resultados e uso de dados fora do Tenant autorizado. `QUALITY_PENDING` é entrega declarada, com elegibilidade restrita; entrega parcial continua somente como `SUCCEEDED_PARTIAL` declarado e revalidado.
...
- Validação estrutural, factual e de variedade; `BriefValidationReport` por briefing; repair limitado; publicação de `QUALITY_PENDING` exclusivamente conforme ADR-027, sem completar quantidade com conteúdo inválido.
- Persistência de `ProductStrategy`, `ContentPlan`, `ContentOpportunity`, `Content` e `ContentBriefVersion` inicial em `DRAFT` ou `QUALITY_PENDING`, com proveniência suficiente para rastrear o job, Product, Strategy e Skill.
...
| Limite de repair | Uma tentativa por parte marcada `REVIEW`; sem segundo repair e sem re-Judge | O contrato aprovado remove rounds globais e preserva a parte original quando o repair falha | Sim, contrato compartilhado `judge-semantico-contrato-a` e design 2026-09-18 |
...
| Conflito sobre gates | Hard gate factual/estrutural e Variety Gate permanecem determinísticos e fora do LLM; `CONTENT_QUALITY_JUDGE` é uma exceção semântica interna após o hard gate, limitada a hook, development, script, CTA e cenas. `PASS` preserva a parte, apenas `REPAIR` chama repair e consome no máximo 2 rounds, e `REJECT` é terminal. Após os dois rounds, apenas `REPAIR` remanescente de item ainda hard-válido, com cenas e variedade válidas, publica `QUALITY_PENDING`; todo `REJECT`, gate inválido ou falha de schema/provider não publica. | Separa segurança/factualidade determinísticas de não convergência editorial sem transferir autoridade ao LLM; `QUALITY_PENDING` não é aprovação nem elegível para lote. | Sim, por ADR-027 |
| Memória na primeira geração | Usar `ProductMemorySnapshot` vazio, sem consultar histórico/recorrência; após sucesso persistir sinais estruturados de todo Content entregue, incluindo `QUALITY_PENDING`; falha/cancelamento não atualizam memória | Conteúdo publicado é entrega para quota e sinal `gerado`; o status impede aprovação/gravação até resolução posterior | Sim, ADR-027 |
...
**Decisões em aberto:** permanece configurável somente a oferta de cancelamento seguro; o limite semântico está fixado em uma tentativa por parte `REVIEW`, sem segundo repair ou re-Judge.
...
O Router resolve e registra o `IntelligenceTier`, provider lógico, modelo lógico, reasoning efetivo e versão das instruções, mesmo quando o MVP usa um único provider. Provider e modelo não são escolhidos pela capability nem variam por plano comercial. Após o hard gate factual/estrutural, `CONTENT_QUALITY_JUDGE` avalia internamente apenas hook, development, script, CTA e cenas; permite repair seletivo por no máximo 2 rounds. `REJECT` é terminal e não publica. Após os dois rounds, `REPAIR` remanescente só publica `QUALITY_PENDING` se o item continuar schema/hard/cenas-válido e o conjunto passar Variety Gate; caso contrário é faltante ou falha fechada conforme ADR-021. `SUCCEEDED` preserva exact-N de Contents publicáveis. Variety Gate e memória não usam judge LLM. O Judge recebe no máximo 3 Contents homogêneos por chamada e devolve decisão por `contentId`; repair agrupa somente a mesma parte e o mesmo round (até 3 itens, ou 2 para development; cenas individuais), conforme ADR-025.
...
O Brief Generator produz `developmentSchemaVersion: 2` e `development: DevelopmentBullet[]`, em que cada bullet é `{ text, action, rationale, factRefs, cta }`. `factRefs` é não vazio, contém somente referências autorizadas do catálogo de evidências e nunca `product:name`; `rationale` explica o vínculo de `text` com esses fatos e não introduz claim novo. `action` preserva o repertório determinístico de comunicação. `cta` é obrigatória em **todo** bullet desde a primeira geração e em todo repair, é orientada à ação e persuasiva sem criar urgência, preço, disponibilidade ou benefício não suportado. A `cta` raiz do briefing continua sendo o fechamento geral e não substitui a CTA de nenhum bullet.
...
O resultado final possui `targetContentCount` Contents publicáveis no `SUCCEEDED`: cada um é `DRAFT` se o Judge concluiu `PASS`, ou `QUALITY_PENDING` somente na exceção do ADR-027. Quando 0 < D < N itens forem publicáveis e F = N−D ≤ `PARTIAL_FAILURE_CAP`, o job conclui `SUCCEEDED_PARTIAL`: publica os D Contents `DRAFT`/`QUALITY_PENDING`, revalida o Variety Gate sobre o subconjunto com teto recomputado `ceil(D/K)` (drop determinístico de excedente se necessário), registra `expectedCount`/`deliveredCount`/`failedCount`/`qualityPendingCount` e assinatura residual por item, e confirma quota pelos D entregues liberando N−D (ADR-021/027). D=0, F>CAP ou falha anterior aos briefs → `FAILED`. Cada `Content` mantém identidade estável, `productId`, `planId`, `opportunityId` quando aplicável, `currentBriefVersionId` e `approvedBriefVersionId` ausente.

- Na v1, `angle`, `hook`, `development`, `script`, `scenes` e `cta` são obrigatórios; `development` é `DevelopmentBullet[]` conforme B-003-07 e cada bullet tem sua própria CTA obrigatória. `structure`, `objective`, `targetAudience`, `pain`, `desire`, `objection`, `benefit` e `notes` são opcionais conforme a oportunidade. O Briefing distingue decisão estratégica de fala sugerida, mantém cenas simples para creator comum, usa linguagem oral e não exige leitura literal do script. Aprovação ou descarte pertencem a Content Operations.
### B-003-09 — Quality Gate, Variety Gate e repair

Antes da publicação do resultado, cada briefing passa pelo hard gate estrutural, factual e de plataforma, cenas válidas, e o conjunto pelo Variety Gate estruturado, todos determinísticos nesta primeira geração. Depois, `CONTENT_QUALITY_JUDGE` avalia hook, development, script, CTA e cenas por parte; somente `REPAIR` entra em `CONTENT_PART_REPAIR` e consome rounds, enquanto `REJECT` é terminal. Após dois rounds, `REPAIR` remanescente não é faltante apenas quando o item e o conjunto continuam integralmente válidos nos gates determinísticos: então publica como `QUALITY_PENDING`, com diagnóstico sanitizado (ADR-027). `REJECT`, hard gate inválido, cena inválida, schema/provider falho ou variedade inválida nunca publicam. Devem ser detectáveis campos obrigatórios ausentes, relações/IDs inválidos, quantidade incorreta, claims sem suporte por evidência, claims contraditos, quantidade de cenas válida para o formato, duplicatas exatas/normalizadas, duplicata por hash de estrutura e concentração funcional.
...
O `BriefValidationReport` registra os resultados objetivos (`factualStatus`, `structuralStatus`, `platformStatus`, `varietyStatus`, `issues`) e diagnósticos sanitizados por bullet separados da auditoria semântica. O diagnóstico por bullet contém somente índice, códigos/flags allowlisted, contagens e partes de claim allowlisted; nunca texto bruto, fatos, prompt ou resposta do provider. Sua chave canônica é `briefId`, derivada de `contentId + briefVersionId`. A auditoria semântica registra `contentId`, round inicial, cinco partes, índice de bullet quando a parte for `development`, criterion e reason allowlisted; seus status são apenas `PASS|REVIEW`. Não existem `REJECT`, `unsupported_persuasion` ou `QUALITY_PENDING`.

O sistema não cria conteúdo irrelevante apenas para atingir a quantidade. Item não publicável após repair conta como F: sem item publicável, F acima de `PARTIAL_FAILURE_CAP` ou conjunto incapaz de fechar com consistência, o job fica `FAILED`; dentro do teto, o job conclui `SUCCEEDED_PARTIAL` e o creator completa somente os F faltantes com `Gerar faltantes`. Um `QUALITY_PENDING` já é entregue, conta em D e não cria faltante, retry, round ou chamada adicional (ADR-021/027).
### B-003-10 — Persistência, idempotência e observabilidade

No sucesso pleno ou parcial (`SUCCEEDED`/`SUCCEEDED_PARTIAL`), uma transação curta fenced persiste Strategy, Plan, Opportunities, os Contents entregues, BriefVersions, relatórios de validação, sinais estruturados da geração, proveniência, `IntelligenceRun` e confirmação do uso reservado pelos D itens entregues, liberando N−D. `QUALITY_PENDING` é persistido nessa mesma transação, conta em D/quota/memória e registra somente `qualityPending`, partes `REPAIR`, rounds e critérios/motivos allowlisted; não persiste payload ou razão livre. `expectedCount = deliveredCount + failedCount`; `qualityPendingCount` é subconjunto de D. A persistência não cria duas Strategies `ACTIVE`, dois Plans equivalentes ou Briefings duplicados para o mesmo job. Em `FAILED`, o `IntelligenceRun` com assinatura residual também é persistido para diagnóstico. CAS perdido por owner/attempt não persiste resultado, run, quota nem memória.
...
Em `SUCCEEDED`, mostra que o Product está pronto e oferece `Revisar conteúdos`; Contents `QUALITY_PENDING` aparecem nessa revisão com o estado “Revisão de qualidade pendente” e copy curta de que fatos e estrutura passaram, mas a melhoria semântica não convergiu. Não mostra partes, rounds, provider, modelo, prompt ou motivo bruto; não oferece aprovação ou inclusão em lote. Em `SUCCEEDED_PARTIAL`, mostra "D de N conteúdos prontos", informa somente os F faltantes com motivo sanitizado por item e oferece `Gerar faltantes` além de `Revisar conteúdos`; Contents não publicados permanecem invisíveis. Este slice não oferece reanálise do mesmo Product `READY`; nova geração/recorrência do Product pertence ao Slice 008. Em `FAILED`, mostra falha recuperável e oferece `Tentar novamente`. O indicador não mostra percentual inventado, ETA, logs, tokens, prompts, provider, modelo, tiers ou detalhes internos.
...
- job `SUCCEEDED` com Strategy, Plan e N Contents publicáveis (`DRAFT` ou `QUALITY_PENDING`): `READY`;
- job `SUCCEEDED_PARTIAL` com Strategy, Plan e D Contents publicáveis: `READY` (com ação adicional `Gerar faltantes`, ADR-021);
- último job sem sucesso (`FAILED` ou `CANCELLED` sem resultado final): `FAILED`.

Essa readiness alimenta os badges dos Product cards e o filtro `Pendente` na lista de Produtos. Product `READY` não oferece reanálise neste slice. Ações de resultado bem-sucedido, qualificadas por estado: após `SUCCEEDED` pleno, `Revisar conteúdos`; após `SUCCEEDED_PARTIAL`, `Revisar conteúdos` + `Gerar faltantes` (ADR-021). `QUALITY_PENDING` não acrescenta ação automática e não é elegível para aprovação/gravação. Nova geração/recorrência arbitrária pertence ao Slice 008.
...
`targetContentCount` é um inteiro validado entre `1` e `30` e permanece estável durante o job. Um job `SUCCEEDED` materializa exatamente essa quantidade de Contents publicáveis: `DRAFT` ou, exclusivamente conforme ADR-027, `QUALITY_PENDING`. Um `SUCCEEDED_PARTIAL` materializa somente os D Contents publicáveis, dentro do teto de falhas do ADR-021.
...
Todo Content final pertence ao Product e ao Plan, começa em `DRAFT`, salvo a exceção `QUALITY_PENDING` do ADR-027, possui uma `ContentBriefVersion` v1 imutável e mantém `approvedBriefVersionId` ausente. `QUALITY_PENDING` não é aprovação e não pode integrar `RecordingBatch`; o slice não aprova, descarta, edita ou regenera Contents.
...
O sistema não publica Strategy, Plan ou Briefings parciais como resultado final **sem declaração**: ou o conjunto inicial fecha completo e o job é `SUCCEEDED` com N Contents publicáveis (`DRAFT` ou `QUALITY_PENDING`), ou fecha parcial declarado como `SUCCEEDED_PARTIAL` com D Contents publicáveis e variedade revalidada (ADR-021), ou o job permanece em processamento/falha de forma recuperável.

### RI-003-11 — Repair limitado

Repair recebe as causas dos rejeitos, preserva passados válidos e possui limite finito configurável. Após dois rounds, `REPAIR` remanescente de item que ainda passou schema, hard gate, cenas e variedade persiste como `QUALITY_PENDING`; todo outro item não convergido não publica: com conteúdos entregues dentro do teto o job fecha `SUCCEEDED_PARTIAL`; sem entregues ou acima do teto, o job falha. Nunca preenche a quantidade com Briefing inválido ou irrelevante (ADR-021/027).
...
Estados do Job, readiness do Product e estado de revisão do Content são dimensões distintas. `SUCCEEDED` não significa `APPROVED`; `DRAFT` e `QUALITY_PENDING` não significam aprovação. `QUALITY_PENDING` é publicado e conta como entregue, mas não é elegível para aprovação/gravação até resolução em Content Operations.

### RI-003-16 — Retry técnico versus retry explícito

Reconnect, reentrada e reclaim de worker usam o mesmo `job.id`, a mesma chave lógica e a mesma reserva. `Tentar novamente` após `FAILED` ou `CANCELLED` cria novo job, nova chave e nova reserva; o job terminal anterior permanece preservado e o Product/fatos são reutilizados. `Gerar faltantes` após `SUCCEEDED_PARTIAL` cria novo job com reserva apenas da quantidade realmente faltante (ADR-021); `QUALITY_PENDING` não é faltante nem dispara retry automático.

### RI-003-17 — Memória inicial e sinais

O snapshot de entrada da primeira geração é vazio e não consulta histórico nem recorrência. Job `SUCCEEDED` ou `SUCCEEDED_PARTIAL` persiste os sinais estruturados de todo Content entregue, incluindo `QUALITY_PENDING`; itens não entregues não sinalizam. Falha ou cancelamento não atualizam a memória.
...
| `GEN-SCHEMA` | Saída não obedece schema canônico | Falhar fechado; schema inválido nunca pode virar `QUALITY_PENDING`. Dentro do teto, o item conta como faltante em `SUCCEEDED_PARTIAL`; fora dele reprova o job. |
| `GEN-FACT` | Claim `UNSUPPORTED` ou `CONTRADICTED` | Remover/corrigir o não suportado; rejeitar o contradito e enviar causa ao repair; item sem hard gate factual aprovado nunca publica, inclusive como `QUALITY_PENDING`. |
...
| `GEN-REPAIR-EXHAUSTED` | Limite de repair atingido | Após dois rounds, publicar `QUALITY_PENDING` somente para `REPAIR` semântico remanescente que ainda passou schema, hard gates, cenas e variedade; `REJECT`, gate inválido e falha schema/provider continuam não publicáveis. Itens realmente não publicáveis seguem `SUCCEEDED_PARTIAL` com D>0 e F ≤ `PARTIAL_FAILURE_CAP`, ou `FAILED` (ADR-021/027). |
...
| `succeeded` | Job `SUCCEEDED` com Strategy, Plan e N Contents publicáveis: readiness `READY`; indicador oferece `Revisar conteúdos`. `QUALITY_PENDING` aparece como `Revisão de qualidade pendente`, sem aprovação ou gravação. |
| `succeeded-partial` | Job `SUCCEEDED_PARTIAL` com D de N Contents publicáveis: readiness `READY`; indicador mostra "D de N prontos", motivo sanitizado por item faltante e oferece `Gerar faltantes` além de `Revisar conteúdos` (ADR-021). `QUALITY_PENDING`, se presente, é entregue e não é faltante. |
...
21. **WHEN** um job terminar em `SUCCEEDED`, **o sistema SHALL** persistir exatamente `targetContentCount` Contents publicáveis: `DRAFT` para Judge `PASS`, ou `QUALITY_PENDING` apenas para `REPAIR` semântico não convergido após dois rounds que ainda passou schema, hard gate, cenas e variedade (ADR-027). **WHEN** terminar em `SUCCEEDED_PARTIAL`, **o sistema SHALL** persistir somente os D Contents publicáveis, revalidar variedade com `ceil(D/K)`, registrar `expectedCount`/`deliveredCount`/`failedCount`/`qualityPendingCount`, confirmar quota pelos D entregues e liberar o restante (ADR-021/027).
22. **WHEN** cada Content inicial for persistido, **o sistema SHALL** manter identidade, `productId`, `planId`, `opportunityId` quando aplicável, `currentBriefVersionId` e status `DRAFT` ou `QUALITY_PENDING` conforme o contrato.
23. **WHEN** cada Content inicial for persistido, **o sistema SHALL** deixar `approvedBriefVersionId` ausente e criar uma `ContentBriefVersion` v1 imutável; `QUALITY_PENDING` SHALL ser inelegível para aprovação e `RecordingBatch`.
...
27. **WHEN** schema validation, hard gate factual/estrutural ou Variety Gate forem executados, **o sistema SHALL** tratá-los como determinísticos fora do Model Router; `CONTENT_QUALITY_JUDGE` avalia internamente hook, development, script, CTA e cenas após o hard gate. **IF** uma parte receber `REPAIR`, **o sistema SHALL** repará-la seletivamente em até 2 rounds; **IF** receber `REJECT`, **o sistema SHALL** impedir publicação. Depois dos dois rounds, o sistema SHALL publicar `QUALITY_PENDING` somente para `REPAIR` remanescente que passe todos os gates determinísticos, cenas e variedade; schema/provider/hard gate inválido nunca publica. O sistema também SHALL validar `ContentSceneSet` disponível e válido antes da publicação.
...
29. **WHEN** um briefing contiver claim `UNSUPPORTED` ou `CONTRADICTED`, **o sistema SHALL** avaliá-lo contra fatos/evidências estruturados, remover/corrigir o não suportado, rejeitar o contradito e impedir resultado factual inválido, inclusive `QUALITY_PENDING`.
30. **WHEN** o conjunto contiver duplicata exata/normalizada ou repetição estrutural indevida, **o sistema SHALL** acionar repair com as causas, preservar Briefings `PASS` e não inventar oportunidade irrelevante; sem Variety Gate válido, não poderá publicar `QUALITY_PENDING`.
32. **IF** o limite de repair for atingido, **o sistema SHALL** aplicar ADR-027 antes do contrato ADR-021: `REPAIR` semântico elegível vira `QUALITY_PENDING`; os demais itens não publicáveis seguem `SUCCEEDED_PARTIAL` dentro do teto ou `FAILED`. O sistema não cria chamada, round, fallback ou retry adicional.
...
35. **WHEN** o job for finalizado, **o sistema SHALL** persistir metadata por capability/batch de duração, tamanhos, retries, validações, repairs e códigos internos, sem prompt completo ou payload bruto; para `QUALITY_PENDING`, somente partes `REPAIR`, rounds e critérios/motivos allowlisted.
...
37. **WHEN** o job atingir `SUCCEEDED` ou `SUCCEEDED_PARTIAL`, **o sistema SHALL** derivar readiness `READY`, disponibilizar D Contents publicáveis e oferecer `Revisar conteúdos`; `QUALITY_PENDING` SHALL ter estado/copy explícitos e permanecer inelegível para aprovação/gravação. `SUCCEEDED_PARTIAL` SHALL oferecer `Gerar faltantes` somente para F itens não publicados (ADR-021/027).
...
- [ ] Um job bem-sucedido entrega Strategy, Plan e a quantidade solicitada de Contents/Briefings publicáveis: `DRAFT` ou `QUALITY_PENDING` sob a exceção estrita do ADR-027; `SUCCEEDED_PARTIAL` entrega D e oferece `Gerar faltantes` apenas para F não publicados.
...
- [ ] Gates classificam claims por evidência; `REJECT`, hard gate inválido, cenas inválidas e schema/provider falho nunca publicam; `REPAIR` elegível após dois rounds publica somente como `QUALITY_PENDING`, sem chamada extra.
...
- [ ] Nenhuma geração ultrapassa capacidade mensal, escopo de Tenant, CSRF ou regras de factualidade; memória recebe sinais somente de Contents entregues, inclusive `QUALITY_PENDING`.
...
- A primeira geração usa snapshot de memória vazio, não consulta histórico/recorrência, persiste sinais estruturados de todos os Contents entregues — inclusive `QUALITY_PENDING` — após sucesso pleno ou parcial (ADR-021/027) e não atualiza memória em falha/cancelamento.
- A primeira geração cria Strategy v1, ContentPlan, oportunidades e a quantidade solicitada de Contents com BriefVersions v1 iniciais em `DRAFT`, salvo `QUALITY_PENDING` exclusivamente sob ADR-027; `approvedBriefVersionId` permanece ausente.
...
- O hard gate factual/estrutural e o Variety Gate permanecem fora do LLM. `CONTENT_QUALITY_JUDGE` é a exceção interna limitada após o hard gate: avalia hook, development, script, CTA e cenas; somente `REPAIR` aciona repair seletivo em no máximo 2 rounds e `REJECT` é terminal. Depois dos dois rounds, somente `REPAIR` semântico de Content ainda schema/hard/cenas/variedade-válido publica como `QUALITY_PENDING`; qualquer outro não-`PASS` é faltante no ADR-021 ou falha fechada. Embeddings, similaridade semântica, deduplicação semântica e judge de variedade/memória ficam fora.
...
- `IntelligenceRun` registra metadata allowlisted por capability/batch — duração, tamanhos, retries, validações, repairs e códigos — sem prompts completos ou payloads brutos; para `QUALITY_PENDING`, registra somente partes `REPAIR`, rounds e critérios/motivos allowlisted.
- `QUALITY_PENDING` é estado persistido/API explícito de Content: conta como entrega para quota, não é faltante, não inicia retry, não é aprovação e não é elegível para `RecordingBatch`.
...
9. Contents exibe exatamente os Briefings publicados após `SUCCEEDED` ou `SUCCEEDED_PARTIAL`: `DRAFT` e, quando aplicável, `QUALITY_PENDING` com rótulo “Revisão de qualidade pendente”, sem expor diagnóstico interno e sem oferecer aprovação/gravação; cada item mostra Hook, Ângulo, Roteiro, Cenas e CTA.

...
- Validação estrutural, factual e de variedade; `BriefValidationReport` por briefing; CTAs proativas e persuasivas por bullet; repair limitado e sem completar quantidade com conteúdo inválido.
- Persistência de `ProductStrategy`, `ContentPlan`, `ContentOpportunity`, `Content` e `ContentBriefVersion` inicial em `DRAFT`, com proveniência estruturada por bullet e suficiente para rastrear o job, Product, Strategy e Skill.
...
- Embeddings, banco vetorial, similaridade semântica, deduplicação semântica e judge LLM de variedade ou memória ficam fora do Slice 003; a variedade usa dimensões estruturadas e normalização determinística. `CONTENT_QUALITY_JUDGE` é a exceção interna: executa uma vez após os hard gates objetivos, limitado a hook, development, script, CTA e cenas, e retorna `PASS|REVIEW`. `PASS` mantém a parte; `REVIEW` chama `CONTENT_PART_REPAIR` no máximo uma vez somente para a parte marcada. Não há re-Judge; falha/schema/inaplicabilidade preserva a parte original. Quando os gates objetivos passam, o Content segue em `DRAFT`; REVIEW semântico não cria faltante, aviso, `QUALITY_PENDING` ou bloqueio. Faltantes e `SUCCEEDED_PARTIAL` decorrem somente de falha objetiva conforme ADR-021.
...
| Conflito sobre gates | Hard gates objetivos factual/estrutural, relação, cardinalidade e Variety Gate permanecem determinísticos e bloqueantes. `CONTENT_QUALITY_JUDGE` é uma exceção semântica interna após os hard gates, limitada a hook, development, script, CTA e cenas, e retorna somente `PASS|REVIEW`. Há uma avaliação inicial; `PASS` preserva a parte; `REVIEW` aciona no máximo um repair somente da parte marcada; não há re-Judge. Falha, schema inválido ou impossibilidade de aplicar repair mantém a parte original. Quando os gates objetivos passam, o resultado é `DRAFT`; Judge não reprova item nem cria faltante, aviso, `QUALITY_PENDING` ou bloqueio por estilo. `unsupported_persuasion` e `REJECT` deixam de existir no contrato semântico. | Separa qualidade subjetiva da autoridade objetiva e mantém o primeiro resultado revisável sem transformar preferência em falha de entrega | Sim, contrato compartilhado `judge-semantico-contrato-a` e design 2026-09-18 |
...
O Router resolve e registra o `IntelligenceTier`, provider lógico, modelo lógico, reasoning efetivo e versão das instruções, mesmo quando o MVP usa um único provider. Provider e modelo não são escolhidos pela capability nem variam por plano comercial. Após os hard gates objetivos, `CONTENT_QUALITY_JUDGE` avalia internamente apenas hook, development, script, CTA e cenas uma vez; retorna `PASS|REVIEW`. Cada `REVIEW` pode chamar `CONTENT_PART_REPAIR` uma única vez para a parte marcada. Não há re-Judge; falha, schema inválido ou aplicação impossível mantém a parte original. Quando os gates objetivos passam, o resultado permanece `DRAFT`. Variety Gate e memória não usam judge LLM. O Judge recebe no máximo 3 Contents homogêneos por chamada e devolve decisão por `contentId`; repair agrupa somente a mesma parte e contexto (até 3 itens, ou 2 para development, cenas individualmente).
...
- Antes da publicação, cada briefing passa pelos hard gates objetivos estrutural, factual, de relação, cardinalidade e plataforma, e o conjunto passa pelo Variety Gate determinístico. Falha objetiva bloqueia o item/job conforme ADR-021. O hard gate diagnostica cada `DevelopmentBullet` por índice, incluindo `factRefs`, claims não suportados e CTA ausente ou factualmente inválida. Depois, `CONTENT_QUALITY_JUDGE` avalia uma vez `hook`, `development`, `script`, `cta` e cenas por parte, retornando somente `PASS|REVIEW`; em `development`, sua evidência semântica é indexada por bullet. `PASS` preserva a parte; `REVIEW` aciona no máximo um `CONTENT_PART_REPAIR` somente para a parte e os índices marcados. Não há Judge na segunda passada. Se o repair falhar, retornar schema inválido ou não puder ser aplicado, a parte original permanece. Quando os hard gates objetivos passam, o Content é persistido em `DRAFT`, mesmo com fallback semântico.
...
Repair não altera IDs, ownership, quota, Strategy ativa ou quantidade do plano. A aplicação valida cada retorno individualmente e recompõe somente a parte e os bullets marcados; erro preserva os valores originais. Faltantes e `SUCCEEDED_PARTIAL` decorrem apenas de falha objetiva ou composição objetivamente inválida, nunca de `REVIEW` semântico.
...
Repair é seletivo: recebe somente as partes marcadas `REVIEW`, preserva as partes `PASS` e tem no máximo uma tentativa por parte. Ao esgotar ou falhar, a parte original permanece; não há re-Judge. Se a composição continuar válida nos hard gates objetivos, o Content é entregue em `DRAFT`. Nunca preenche a quantidade com Briefing objetivamente inválido ou irrelevante; somente falha objetiva pode produzir faltante ou `SUCCEEDED_PARTIAL` (ADR-021).
...
Estados do Job, readiness do Product e estado de revisão do Content são dimensões distintas. `SUCCEEDED` não significa `APPROVED`; `DRAFT` não significa job em execução.
...
| `GEN-SCHEMA` | Saída não obedece schema canônico | Rejeitar/corrigir dentro da validação objetiva; em repair semântico, preservar a parte original quando o retorno for inválido. O item só conta como faltante se a composição falhar hard gate objetivo (ADR-021). |
| `GEN-FACT` | Claim `UNSUPPORTED` ou `CONTRADICTED` | Remover/corrigir o não suportado; bloquear o contradito no hard gate e aplicar a causa objetiva conforme ADR-021. |
| `GEN-VARIETY` | Duplicata normalizada ou concentração estrutural indevida | Bloquear o item/conjunto no Variety Gate e acionar repair objetivo; no fechamento parcial, revalidar o subconjunto com `ceil(D/K)` (ADR-021). |
| `GEN-REPAIR-EXHAUSTED` | Falha objetiva após a validação/repair | Com D>0 e F ≤ `PARTIAL_FAILURE_CAP`: `SUCCEEDED_PARTIAL` publicando os itens aprovados nos gates objetivos; caso contrário `FAILED`. Repair semântico que falhar usa o fallback original e não produz este código por si só. |
...
21. **WHEN** um job terminar em `SUCCEEDED`, **o sistema SHALL** persistir exatamente `targetContentCount` Contents com status `DRAFT`. **WHEN** terminar em `SUCCEEDED_PARTIAL`, **o sistema SHALL** persistir somente os Contents aprovados nos hard gates objetivos, revalidar variedade com `ceil(D/K)`, registrar `expectedCount`/`deliveredCount`/`failedCount` com assinatura residual por falha objetiva, confirmar quota pelos entregues e liberar o restante (ADR-021). O Judge semântico não cria faltantes.
...
27. **WHEN** schema validation, hard gate factual/estrutural ou Variety Gate forem executados, **o sistema SHALL** tratá-los como determinísticos fora do Model Router; `CONTENT_QUALITY_JUDGE` avalia internamente hook, development, script, CTA e cenas uma vez após os hard gates. **IF** uma parte receber `REVIEW`, **o sistema SHALL** tentar repará-la seletivamente no máximo uma vez; **IF** o repair falhar, retornar schema inválido ou não puder ser aplicado, **o sistema SHALL** manter a parte original e não chamar Judge novamente. Quando os hard gates objetivos passarem, o Content SHALL permanecer `DRAFT`.
...
29. **WHEN** um briefing contiver claim `UNSUPPORTED` ou `CONTRADICTED`, **o sistema SHALL** tratá-lo no hard gate factual contra fatos/evidências estruturados; o Judge semântico não autoriza, rejeita ou reclassifica claims factuais.
...
32. **IF** a validação objetiva final falhar ou houver composição objetivamente inconsistente, **o sistema SHALL** aplicar o contrato de entrega do ADR-021: `SUCCEEDED_PARTIAL` quando houver itens aprovados dentro do teto, ou `FAILED`; repair semântico sem convergência usa a parte original e não causa parcial por si só.
...
- [ ] Gates classificam claims por evidência; o Judge semântico retorna `PASS|REVIEW` uma vez, repaira somente partes `REVIEW` no máximo uma vez e preserva a parte original se falhar. Falhas objetivas continuam tratadas separadamente e usam código específico.
...
- [ ] Nenhuma geração ultrapassa capacidade mensal, escopo de Tenant, CSRF ou regras de factualidade; memória só recebe sinais após sucesso.
...
- O hard gate factual/estrutural e o Variety Gate permanecem fora do LLM. `CONTENT_QUALITY_JUDGE` é a exceção interna limitada após os hard gates: avalia uma vez hook, development, script, CTA e cenas; somente `REVIEW` aciona repair seletivo único. Não há re-Judge; falha/schema/inaplicabilidade preserva a parte original. Quando os gates objetivos passam, o resultado é `DRAFT`; apenas falha objetiva produz faltante/parcial conforme ADR-021. Embeddings, similaridade semântica, deduplicação semântica e judge de variedade/memória ficam fora.
...
- O repair semântico ocorre no máximo uma vez por parte marcada `REVIEW`; não há segundo repair nem re-Judge. Falha, schema inválido ou aplicação impossível preserva a parte original. A oferta de cancelamento seguro permanece configuração em aberto.

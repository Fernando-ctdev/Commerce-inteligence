...
- schema validation, factualidade, hard Quality Gate e Variety Gate, contagens, orquestração, persistência, idempotência, quota e estados → determinísticos, fora do Router; exceção limitada: `CONTENT_QUALITY_JUDGE` e `CONTENT_PART_REPAIR` → `HIGH`, após o hard gate, somente para hook, development, script, CTA e cenas. `REJECT` é terminal; somente `REPAIR` é reparável.

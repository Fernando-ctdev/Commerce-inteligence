...
test("normaliza ImportView server-side e confirma no contrato Product", async () => {
  ...
  try {
    ...
    assert.equal(record.errorCode, "CANDIDATE_EXPIRED");
    assert.equal(record.handoffExpired, true);

    const preferences = { targetContentCount: 20, creatorPresence: "either" as const, constraints: "Sem gírias." };
    const result = await confirmProductImport(record.id, record.version, record.candidate!, preferences);
    ...
    assert.deepEqual(sent.preferences, preferences);
  } finally {
    globalThis.fetch = originalFetch;
  }
});

test("getProductImport encaminha signal para permitir abortar polling", async () => {
  const originalFetch = globalThis.fetch;
  ...
  try {
    await getProductImport("attempt-1", controller.signal);

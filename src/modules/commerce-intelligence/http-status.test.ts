...
test("API brief payload omits legacy scenes and retains development as a separate array", () => { assert.deepEqual(projectBriefPayload({ hook: "Hook", development: ["bullet", "bullet"], script: "Oral", cta: "CTA", scenes: ["legado"] }), { hook: "Hook", development: ["bullet", "bullet"], script: "Oral", cta: "CTA" }); });
test("API rejects invalid development shape instead of normalizing it to an empty array", () => {
  for (const development of [undefined, [], "bullet", ["bullet", 1], ["1", "2", "3", "4", "5", "6", "7"]]) {

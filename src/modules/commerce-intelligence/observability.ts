...
export type JobEventFields = {
  ...
  providerStatus?: number; rootShape?: string; responseKeys?: string[]; arrayLength?: number; endpoint?: string; requestId?: string;
...
export type SanitizedGateReport = { briefId: string; factualStatus: string; claimType: string; evidenceRefs: string[]; structuralStatus: string; platformStatus: string; varietyStatus: string; decision: string; issues: string[] };
export function sanitizeGateReports(reports: Array<Record<string, unknown>>): SanitizedGateReport[] {
  return reports.map((report) => ({
    ...
    issues: Array.isArray(report.issues) ? report.issues.filter((issue): issue is string => typeof issue === "string").map((issue) => issue.slice(0, 200)) : [],
  }));
}
...
const EVENT_ALLOWLIST: Record<JobEventName, (keyof JobEventFields)[]> = {
  ...
  "capability.completed": [...BASE_ALLOWLIST, "durationMs", "timeoutMs", "requestBytes", "trustedContextBytes", "externalBytes", "responseBytes", "providerStatus", "rootShape", "responseKeys", "arrayLength", "retry"],
  "capability.failed": [...BASE_ALLOWLIST, "durationMs", "timeoutMs", "providerStatus", "endpoint", "requestId", "expected", "received", "retry", "item", "issue", "errorKind", "rate"],

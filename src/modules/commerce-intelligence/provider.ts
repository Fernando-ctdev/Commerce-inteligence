...
export const CONTENT_BRIEF_GENERATION_INSTRUCTION =
  "Retorne um objeto JSON raiz com items contendo EXATAMENTE a mesma quantidade de briefings que oportunidades recebidas, um por oportunidade e na mesma ordem. Retorne somente angle, hook, development, script e cta; não retorne scenes nem qualquer campo de cena. Use selectedPatterns[index].hook.text como hook; se adaptar, faça uma variação curta de até 12 palavras. Use categoria no hook somente se explícita em relevantFacts. Development é uma lista de 1 a 4 strings: cada bullet é uma orientação estratégica e acionável sobre o raciocínio que o creator comunica ou prova, não uma lista de features. Use relevantFacts como única fonte de fatos técnicos em development e script; angle e oportunidade orientam o recorte, mas não são fonte de fatos. Escreva script desenvolvendo development; todo fato técnico no script deve estar em relevantFacts e representado em development. Use selectedPatterns[index].cta.text somente para formular cta e mantenha cta separado de hook, development e script. A quantidade de items deve ser exatamente igual à quantidade de oportunidades recebidas; nunca omita, adicione ou duplique. Não inclua contentId, briefVersionId, ownership, status, quota, provider, model, tier ou comandos de workflow.";

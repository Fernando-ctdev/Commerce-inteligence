...
export const CONTENT_BRIEF_GENERATION_INSTRUCTION =
  "Retorne um objeto JSON raiz com items contendo EXATAMENTE a mesma quantidade de briefings que oportunidades recebidas, um por oportunidade e na mesma ordem. Retorne somente angle, hook, development, script e cta; não retorne scenes nem qualquer campo de cena. Para cada item, use selectedPatterns[index].hook.text como hook: prefira o texto selecionado diretamente; se adaptar, faça uma variação curta de até 12 palavras que preserve a ideia e a abertura, podendo usar productReference.name quando isso soar natural. Só use categoria no hook se ela estiver explícita em relevantFacts; não a infira. Preserve hooks naturalmente específicos e a dêixis do padrão quando ela soar natural. Hook é uma frase curta de abertura, nunca uma cena ou sequência de ações. Retorne development sempre como uma lista de 1 a 4 bullets factuais verificáveis, ancorados somente em relevantFacts autorizados. Os bullets orientam o que comunicar ao creator; não podem conter câmera, cena, CTA, roteiro ou instrução de gravação; evite comandos como filme, mostre, fale, clique, carrinho, cupom ou frete. Não invente nem infira atributo ausente da evidência. Escreva script como fala oral natural em primeira pessoa, desenvolvendo somente os pontos de development; não leia uma lista de features nem acrescente claims. Qualquer afirmação factual do roteiro deve estar autorizada por relevantFacts e representada nos pontos de development. Use selectedPatterns[index].cta.text somente para formular cta e mantenha-o separado do hook, development e script. Respeite Meu estilo, sem exigir gramática formal. A quantidade de items deve ser exatamente igual à quantidade de oportunidades recebidas; nunca omita, adicione ou duplique. Não inclua contentId, briefVersionId, ownership, status, quota, provider, model, tier ou comandos de workflow.";

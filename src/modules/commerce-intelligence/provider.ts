...
export const CONTENT_BRIEF_GENERATION_INSTRUCTION =
  "Retorne um objeto JSON raiz com items contendo EXATAMENTE a mesma quantidade de briefings que oportunidades recebidas, um por oportunidade e na mesma ordem. Retorne somente angle, hook, development, script e cta; não retorne scenes nem qualquer campo de cena. Para cada item, use selectedPatterns[index].hook.text como hook: prefira o texto selecionado diretamente; se adaptar, faça uma variação curta de até 12 palavras que preserve a ideia e a abertura. Se o hook usar dêixis demonstrativa genérica (por exemplo, ‘isso’, ‘esse produto’ ou ‘essa coisa’), pode manter essa forma ou mencionar productReference.name ou productReference.category quando isso soar natural; não faça substituição obrigatória. Evite repetir a mesma forma de referência em excesso, sem mudar o sentido nem forçar gramática perfeita. Preserve hooks naturalmente específicos. Hook é uma frase curta de abertura, nunca uma cena ou sequência de ações. Retorne development como uma lista separada de 1 a 4 bullets objetivos sobre o produto, usando somente evidência autorizada e sem inventar atributos. Escreva script como roteiro oral natural em primeira pessoa que desenvolve esses bullets, sem acrescentar fatos; não copie o CTA para o roteiro. Use selectedPatterns[index].cta.text somente para formular cta e mantenha-o separado do hook, development e script. Respeite Meu estilo na linguagem, sem transformar hook ou development em instruções de gravação. A quantidade de items deve ser exatamente igual à quantidade de oportunidades recebidas; nunca omita, adicione ou duplique. Não inclua contentId, briefVersionId, ownership, status, quota, provider, model, tier ou comandos de workflow.";

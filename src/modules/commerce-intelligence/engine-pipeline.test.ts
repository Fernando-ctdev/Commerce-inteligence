...
test("composes validated provider outputs into the pipeline (4 foundational + batched briefs)", async () => {
  const calls: string[] = [];
  const router = { describe, complete: async (task: string) => { calls.push(task); if (task === "PRODUCT_UNDERSTANDING") return understanding; if (task === "COMMERCIAL_OPPORTUNITY_MAPPING") return envelope; if (task === "STRATEGY_SYNTHESIS") return strategyPayload; if (task === "CONTENT_PLAN_GENERATION") return { opportunities: [contentOpportunity] }; if (task === "CONTENT_BRIEF_GENERATION") return { items: [{ angle: "demonstração", hook: "Veja", development: ["Destaque o tecido respiravel para explicar como o tecido respiravel afeta o uso"], script: "Mostre o Produto", cta: "Confira" }] }; return {}; } };
...
test("repairs only rejected briefs via causal CONTENT_BRIEF_GENERATION, preserving ids", async () => {
  ...
  let briefGenCount = 0;
  const router = { describe, hash: () => "h", complete: async (task: string, input?: { trustedContext?: unknown }) => { if (task === "PRODUCT_UNDERSTANDING") return understanding; if (task === "COMMERCIAL_OPPORTUNITY_MAPPING") return envelope; if (task === "STRATEGY_SYNTHESIS") return strategyPayload; if (task === "CONTENT_PLAN_GENERATION") return { opportunities: [contentOpportunity] }; if (task === "CONTENT_BRIEF_GENERATION") { briefGenCount++; if (briefGenCount > 1) repairContexts.push(input?.trustedContext as Record<string, unknown>); briefCalls.push("gen"); if (briefGenCount === 1) return { items: [{ angle: "x", hook: "h", development: ["Carga de 999 kg em teste"], script: "testado com 999 kg de carga", cta: "c" }] }; return { items: [{ angle: "dem", hook: "Veja", development: ["Destaque o tecido respiravel para explicar como o tecido respiravel afeta o uso"], script: "Mostre o Produto na prática", cta: "Confira" }] }; } return {}; } };
  const result = await runFirstGeneration({ productId: "p", jobId: "j", name: "Produto", description: "Tecido respirável", targetContentCount: 1, router });
  ...
  assert.ok(repairContexts[0]);
  assert.ok((repairContexts[0].causes as string[][])[0].length > 0);
  assert.deepEqual((repairContexts[0].repairContrast as Array<Record<string, unknown>>)[0], {
    featureList: "Tecido respirável",
    actionWithReason: "Comente Tecido respirável para explicar por que esse fato importa para o angulo.",
  });
});
test("batches brief generation sequentially with server-derived ids", async () => {
  const calls: string[] = [];
  const router = { describe, complete: async (task: string) => { calls.push(task); if (task === "PRODUCT_UNDERSTANDING") return understanding; if (task === "COMMERCIAL_OPPORTUNITY_MAPPING") return envelope; if (task === "STRATEGY_SYNTHESIS") return strategyPayload; if (task === "CONTENT_PLAN_GENERATION") return { opportunities: [contentOpportunity, contentOpportunity] }; if (task === "CONTENT_BRIEF_GENERATION") return { items: [{ angle: "a", hook: "h", development: ["Destaque o tecido respiravel para explicar como o tecido respiravel afeta o uso"], script: "Produto com demonstração", cta: "c" }, { angle: "a2", hook: "h2", development: ["Destaque o tecido respiravel para explicar como o tecido respiravel afeta o uso"], script: "Produto na prática", cta: "c2" }] }; return {}; } };
...
test("mapping context is compact and allowlisted without strategy plan skill or raw facts", async () => {
  ...
  await runFirstGeneration({ productId: "p", jobId: "j", name: "Produto", description: "Tecido respirável", facts: { features: ["x"], rawAggregate: ["não enviar"], memoryHistory: ["nada"], discountPercentage: "20% de desconto" }, targetContentCount: 1, router });
  ...
  assert.deepEqual((capturedContext as { product: Record<string, unknown> }).product, { name: "Produto", description: "Tecido respirável", category: undefined, brand: undefined, priceAmount: undefined, priceCurrency: undefined, discountPercentage: "20% de desconto" });
});
...
test("plan and brief contexts expose only their explicit allowlisted slices", async () => {  let planContext: Record<string, unknown> | undefined;
  ...
  assert.deepEqual(briefContext.opportunities, [{ angle: "demonstração", hookMechanism: "prova", noveltyTargets: ["angle"] }]);

...
test("brief_validation_reports ganhou claimType/evidenceRefs nullable sem backfill", async (t) => {
  ...
  const legacy = await prisma.$queryRaw<Array<{ n: bigint }>>`SELECT COUNT(*)::bigint AS n FROM brief_validation_reports WHERE "claimType" IS NOT NULL OR "evidenceRefs" IS NOT NULL`;
  assert.equal(Number(legacy[0].n), 0, "nenhum valor inventado em relatórios anteriores");
});

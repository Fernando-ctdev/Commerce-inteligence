...
test("rejects excessive scene count for platform", () => { process.env.GENERATION_SCENES_ENABLED = "1"; try { assert.equal(validateBriefSet([{ ...base, scenes: ["1", "2", "3", "4", "5", "6", "7"] }], evidence)[0].platformStatus, "FAIL"); } finally { delete process.env.GENERATION_SCENES_ENABLED; } });

...
test("objective claim with matching value and unit is supported without literal substring", () => { const ev = { facts: ["Bateria de 4000 mAh"], refs: ["fact:bateria"] }; const report = validateBriefSet([{ ...base, development: ["Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada", "Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada"], script: "rende até 4000 mAh em uso intenso contínuo" }], ev)[0]; assert.equal(report.factualStatus, "SUPPORTED"); assert.equal(report.decision, "PASS"); });
test("objective claim with conflicting value is contradicted", () => { const ev = { facts: ["Bateria de 4000 mAh"], refs: ["fact:bateria"] }; const report = validateBriefSet([{ ...base, development: ["Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada", "Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada"], script: "nada supera os 5000 mAh reais" }], ev)[0]; assert.equal(report.factualStatus, "CONTRADICTED"); assert.equal(report.decision, "REJECT"); });
test("strategic claim without technical attribute is inferred but safe", () => assert.equal(validateBriefSet([{ ...base, development: ["Destaque a escolha inteligente para contextualizar o uso", "Destaque a escolha inteligente para contextualizar o uso"], script: "a escolha inteligente do dia a dia" }], evidence)[0].factualStatus, "INFERRED_BUT_SAFE"));
test("non-numeric objective attribute backed by evidence is supported", () => {
  ...
  const report = validateBriefSet([{ ...base, development: ["Destaque a base magnética para mostrar como a base magnética auxilia o apoio na montagem", "Destaque a base magnética para mostrar como a base magnética auxilia o apoio na montagem"], script: "A base magnética segura o celular durante a montagem" }], ev)[0];
...
test("non-numeric objective attribute without evidence is unsupported", () => {
  const ev = { facts: ["Base magnética, compacto, montagem simples."], refs: ["fact:features"] };
  const report = validateBriefSet([{ ...base, development: ["Destaque a base magnética para mostrar como a base magnética auxilia o apoio na montagem", "Destaque a base magnética para mostrar como a base magnética auxilia o apoio na montagem"], script: "Feito em alumínio aeroespantil" }], ev)[0];
  assert.equal(report.factualStatus, "UNSUPPORTED");
  assert.equal(report.claimType, "objetivo");
  assert.equal(report.decision, "REPAIR");
});
test("contradicted numeric fact still rejected with attribute catalog present", () => {
  ...
  const report = validateBriefSet([{ ...base, development: ["Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada", "Destaque a bateria de 4000 mAh para relacionar 4000 mAh a bateria informada"], script: "são 9000 mAh de autonomia" }], ev)[0];
...
test("rejects unsupported breathability claims in the development or script", () => {
  const report = validateBriefSet([{ ...base, development: ["Destaque que a peça deixa o ar circular", "Destaque que a peça deixa o ar circular"] }], { facts: ["Calça de algodão"], refs: ["fact:product"] })[0];
  assert.equal(report.factualStatus, "UNSUPPORTED");
  assert.equal(report.decision, "REPAIR");
});
test("thermal and pocket-capacity paraphrases require matching relevant facts, not refs", () => {
  ...
  const thermal = validateBriefSet([{ ...base, development: ["Destaque o tecido respirável para mostrar como o tecido respirável ajuda no uso", "Destaque o tecido respirável para mostrar como o tecido respirável ajuda no uso"], script: "O tecido ajuda a ventilar" }], thermalFact)[0];
  ...
  const overclaim = validateBriefSet([{ ...base, development: ["Destaque o uso para contextualizar a escolha", "Destaque o uso para contextualizar a escolha"], script: "O tecido ajuda a ventilar e não esquenta" }], thermalFact)[0];
  ...
  const capacity = validateBriefSet([{ ...base, development: ["Destaque o bolso que comporta telefone para mostrar como a capacidade do bolso atende ao telefone", "Destaque o bolso que comporta telefone para mostrar como a capacidade do bolso atende ao telefone"], script: "No bolso cabe o celular" }], capacityFact)[0];
  assert.equal(capacity.factualStatus, "SUPPORTED");
  assert.equal(capacity.decision, "PASS");
  const oppositeCapacity = validateBriefSet([{ ...base, development: ["Destaque o uso para contextualizar a escolha", "Destaque o uso para contextualizar a escolha"], script: "O bolso não cabe celular" }], capacityFact)[0];
...
test("script claims beyond factual development are repaired", () => {
  ...
  const report = validateBriefSet([{ ...base, development: ["Destaque o produto para contextualizar a compra", "Destaque o produto para contextualizar a compra"], script: "O produto tem garantia vitalícia" }], ev)[0];
  assert.equal(report.decision, "REPAIR");
...
test("development needs a communication action somewhere, not necessarily leading", () => {
  ...
  const nonInitialExperience = validateBriefSet([{ ...base, development: ["A leveza do tecido duna merece destaque: comente a leveza que voce sente com o tecido duna", "Destaque o tecido duna leve e macio porque o toque do tecido duna macio importa no uso"] }], { facts: ["Tecido duna leve e macio"], refs: ["fact:product"] })[0];
...
test("universal pocket capacity claim requires an explicitly universal fact", () => {
  const brief = { ...base, development: ["Destaque o bolso que comporta todo telefone para mostrar como o bolso acomoda o telefone", "Destaque o bolso que comporta todo telefone para mostrar como o bolso acomoda o telefone"], script: "No bolso cabe todo smartphone" };
...
test("product identity cannot support an absolute script claim", () => {
  ...
  const report = validateBriefSet([{ ...base, development: ["Destaque o produto para contextualizar a escolha", "Destaque o produto para contextualizar a escolha"], script: "Esse produto sempre funciona perfeitamente para qualquer pessoa", cta: "Confira no carrinho" }], ev)[0];
  assert.equal(report.factualStatus, "UNSUPPORTED");
  assert.equal(report.decision, "REPAIR");
});
...
test("escopo editorial: CTA de preço/valor exige antecedente no corpo do briefing", () => {
  const d2 = ["Destaque o produto para mostrar o produto no dia a dia", "Destaque o produto em uso para conectar o uso pratico do produto"];
  const semAntecedente = validateBriefSet([{ ...base, development: d2, cta: "Corre pro preço antes que acabe" }], evidence)[0];
  assert.equal(semAntecedente.decision, "REPAIR");
  assert.ok(semAntecedente.issues.includes("CTA de preço/valor sem antecedente no corpo do briefing"));
  const evPreco = { facts: ["Produto", "R$ 29,90"], refs: ["product:name", "fact:price"] };
  const comAntecedente = validateBriefSet([{ ...base, development: [...d2, "Destaque o preço de R$ 29,90 para mostrar o valor do produto"], cta: "Aproveita o preço de R$ 29,90" }], evPreco)[0];
  assert.equal(comAntecedente.decision, "PASS");
});

test("escopo editorial: teto de 2 hooks em pergunta por lote de 5 apenas com alternativas elegíveis", () => {
  const question = (n: number) => ({ ...base, contentId: `q${n}`, briefVersionId: `qv${n}`, hook: `Por que o uso cotidiano do produto ${n} marca no dia a dia?` });
  const alternative = (n: number) => ({ ...base, contentId: `n${n}`, briefVersionId: `nv${n}` });
  const d2 = ["Destaque o produto para mostrar o produto no dia a dia", "Destaque o produto em uso para conectar o uso pratico do produto"];
  const set = (briefs: Array<{ contentId: string }>) => validateBriefSet(briefs, evidence);
  // 3 perguntas + 2 alternativas: só o excesso (3ª) vira REPAIR; perguntas não banidas.
  const comExcesso = set([question(1), question(2), question(3), alternative(1), alternative(2)]);
  const excesso = comExcesso.find((report) => report.issues.includes("excesso de hooks em pergunta no lote"));
  assert.ok(excesso, "apenas o hook em pergunta excedente é marcado");
  assert.equal(comExcesso.filter((report) => report.issues.includes("excesso de hooks em pergunta no lote")).length, 1);
  // 2 perguntas: dentro do teto, nada marcado.
  const noLimite = set([question(1), question(2), alternative(1), alternative(2), alternative(3)]);
  assert.ok(noLimite.every((report) => !report.issues.includes("excesso de hooks em pergunta no lote")));
  // Lote todo em pergunta: sem alternativa elegível, o teto não se aplica.
  const soPerguntas = set([question(1), question(2)]);
  assert.ok(soPerguntas.every((report) => !report.issues.includes("excesso de hooks em pergunta no lote")));
  assert.ok(d2.length === 2); // guarda: development das alternativas permanece 2–6
});

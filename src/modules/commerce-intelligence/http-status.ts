...
const include = { strategies: true, plan: true, metadata: true, contents: { include: { briefs: { orderBy: { version: "asc" as const }, take: 1 }, sceneSets: true } } };
...
export function projectScenes(content: { currentBriefVersionId: string | null; sceneSets?: Array<{ briefVersionId: string; status: string; payload: unknown }> }): { status: string; scenes: Array<{ description: string }>; generated: number; dropped: number } | null {
  ...
  const scenes = Array.isArray(payload.scenes)
    ? payload.scenes.filter((scene): scene is { description: string } => Boolean(scene && typeof scene === "object" && !Array.isArray(scene) && typeof (scene as { description?: unknown }).description === "string"))
    : [];
  return {
...
function partialMissing(old: { status: string; targetContentCount: number; metadata?: unknown }): number | undefined {
...
async function restartFrom(req: Request, id: string, allowed: CommerceIntelligenceJobStatus[], action: "retry" | "complete") {
  ...
  try {
    ...
    const status = code === "GEN-ACTIVE" || code === "GEN-READY" || code === "GEN-PRODUCT-CAPACITY" ? 409 : code === "GEN-CAPACITY" ? 429 : 400;

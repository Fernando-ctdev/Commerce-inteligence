...
export const CARDINALITY_POLICY: Record<string, CardinalityRule> = {
  ...
  communicationRisks: { min: 0, minWithEvidence: 1, max: 8 },
  ...
  scenes: { min: 2, minWithEvidence: 2, max: 8 },
...
// Feature flag: scenes permanece no contrato canônico para a futura AI Content Production,
// mas o fluxo atual do Commerce Intelligence não gera nem depende de scenes (padrão:
// desativado). Ativar com GENERATION_SCENES_ENABLED=1|true restaura o comportamento atual.
export const scenesEnabled = (): boolean =>
  process.env.GENERATION_SCENES_ENABLED === "1" || process.env.GENERATION_SCENES_ENABLED === "true";
export type ContentBriefVersion = { contentId: string; briefVersionId: string; version: 1; angle: string; hook: string; development?: string[]; script: string; scenes?: string[]; cta: string; structure?: string; objective?: string; targetAudience?: string; pain?: string; desire?: string; objection?: string; benefit?: string; notes?: string };
// Contrato do briefing:
// - Modo ativo (flag desativada): development obrigatório (1–4 bullets, decisão do Arquiteto),
//   scenes NUNCA sai no output — mesmo que o provider envie, é descartado (não se depende dele).
// - Modo legado (GENERATION_SCENES_ENABLED): scenes obrigatório, development opcional —
//   preserva a futura AI Content Production sem redefinir o contrato canônico.
export function validateContentBrief(value: unknown): ContentBriefVersion { if (!value || typeof value !== "object") throw new ContractError("GEN-SCHEMA", "Brief inválido"); const v = value as Record<string, unknown>; const scenes = scenesEnabled() ? strings(v.scenes, "scenes") : undefined; const development = v.development === undefined ? (scenesEnabled() ? undefined : strings(v.development, "development")) : strings(v.development, "development"); return { contentId: id(v.contentId, "contentId"), briefVersionId: id(v.briefVersionId, "briefVersionId"), version: 1, angle: text(v.angle, "angle"), hook: text(v.hook, "hook"), development, script: text(v.script, "script", 8_000), scenes, cta: text(v.cta, "cta"), structure: v.structure === undefined ? undefined : text(v.structure, "structure", 100), objective: v.objective === undefined ? undefined : text(v.objective, "objective"), targetAudience: v.targetAudience === undefined ? undefined : text(v.targetAudience, "targetAudience"), pain: v.pain === undefined ? undefined : text(v.pain, "pain"), desire: v.desire === undefined ? undefined : text(v.desire, "desire"), objection: v.objection === undefined ? undefined : text(v.objection, "objection"), benefit: v.benefit === undefined ? undefined : text(v.benefit, "benefit"), notes: v.notes === undefined ? undefined : text(v.notes, "notes") }; }
...
export function validateContentBriefDraft(value: unknown): Omit<ContentBriefVersion, "contentId" | "briefVersionId" | "version"> {
  ...
  const scenes = scenesEnabled() ? strings(v.scenes, "scenes") : undefined;
  const development = v.development === undefined ? (scenesEnabled() ? undefined : strings(v.development, "development")) : strings(v.development, "development");
...
export const structureHash = (brief: Pick<ContentBriefVersion, "structure" | "scenes" | "development" | "cta">): string => { let h = 2166136261; for (const c of JSON.stringify([brief.structure ?? "", brief.scenes?.length ?? brief.development?.length ?? 0, brief.cta]).normalize("NFKC").toLowerCase()) h = Math.imul(h ^ c.charCodeAt(0), 16777619); return (h >>> 0).toString(16).padStart(8, "0"); };

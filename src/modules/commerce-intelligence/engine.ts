...
export async function runFirstGeneration(input: EngineInput): Promise<EngineResult> {
  ...
  const track = async (task: LogicalTask, context: unknown, run: (onMetrics?: (metrics: ProviderCallMetrics) => void) => Promise<Record<string, unknown>>): Promise<Record<string, unknown>> => {
    ...
    try {
      ...
      emitJobEvent("capability.failed", { jobId: input.jobId, attempt, task, tier: ROUTER_MAP[task], model: captured?.model ?? effectiveModel(task), requestBytes: captured?.requestBytes, trustedContextBytes: captured?.trustedContextBytes, externalBytes: captured?.externalBytes, responseBytes: captured?.responseBytes ?? undefined, durationMs, errorName: error instanceof Error ? error.name : "unknown", errorCode, errorKind: typeof safe.errorKind === "string" ? safe.errorKind : undefined, providerStatus: typeof safe.providerStatus === "number" ? safe.providerStatus : undefined, endpoint: typeof safe.endpoint === "string" ? safe.endpoint : undefined, requestId: typeof safe.requestId === "string" ? safe.requestId : undefined, rate: safe.rate && typeof safe.rate === "object" ? safe.rate as Record<string, string> : undefined });
      throw error;
    }
  };

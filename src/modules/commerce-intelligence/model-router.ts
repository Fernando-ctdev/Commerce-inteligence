...
// Métricas sanitizadas por chamada do provider (allowlist espelhada no JSONL e no IntelligenceRun).
export type ProviderCallMetrics = { model: string; reasoning: string; providerStatus: number | null; requestBytes: number; trustedContextBytes: number; externalBytes: number; responseBytes: number | null; durationMs: number };

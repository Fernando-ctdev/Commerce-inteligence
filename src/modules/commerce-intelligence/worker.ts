...
// diagnostics com gateReports/causes e sceneOutcomes — usada pelo catch do
// processGeneration e testável sem worker rodando.
export function buildFailureRun(code: string, stage: string | null, detail: unknown): {
  ...
  const diagnostics = rejected || sceneOutcomes.length
    ? { ...(rejected ?? { gateReports: [], causes: [] }), sceneOutcomes }
    : null;
...
const costPriceResolver = (db: PriceReader) =>
  (provider: string | undefined, model: string | undefined): Promise<PriceSnapshot | null> =>
    provider && model
      ? resolveProviderModelPrice(db, provider, model, new Date())

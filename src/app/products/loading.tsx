...
export default function ProductsLoading() {
  return (
    <ProductShell active="products" action={null} eyebrow="Produtos" title="Seus produtos">
      <div className={listStyles.list}>
        <div className={listStyles.listIntro}>
          ...
          <span aria-hidden="true" className={listStyles.primaryButton} style={{ pointerEvents: "none" }}>
            Adicionar produto
          </span>
        </div>
        <div className={listStyles.controls} role="search">
          <label className={listStyles.searchLabel} htmlFor="product-search">Buscar produtos</label>
          <input className={listStyles.searchInput} disabled id="product-search" placeholder="Nome, categoria ou descrição" type="search" />
          <div aria-label="Filtrar produtos" className={listStyles.filters} role="group">
            {([["all", "Todos"], ["active", "Ativos"], ["archived", "Arquivados"]] as const).map(([value, label]) => (
              <button
        ...
        <ul aria-hidden="true" className={listStyles.cards}>
          {Array.from({ length: 6 }, (_, index) => (
            <li className={listStyles.card} key={index}>
              ...
              <Skeleton className={listStyles.cardAction} style={{ blockSize: 16, inlineSize: 96 }} />
            </li>
          ))}
        </ul>
      </div>
